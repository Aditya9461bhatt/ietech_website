...
export default function AdminDashboard({ onNavigate }: { onNavigate: (route: string) => void }) {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0a0a0a] text-neutral-900 dark:text-white font-sans flex flex-col">
      {/* Header */}
      <div className="bg-black border-b border-neutral-800 p-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button onClick={() => onNavigate('#/')} className="text-neutral-600 dark:text-neutral-500 hover:text-white transition-colors">
      ...
      <div className="flex-1 p-8 max-w-6xl mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row gap-6 justify-between mb-8">
          ...
          <div className="flex gap-2">
            <button 
              ...
              className={`px-4 py-2 text-xs font-bold uppercase tracking-widest border transition-colors ${filter === 'all' ? 'border-[#3F618C] text-[#3F618C] bg-[#3F618C]/10' : 'border-neutral-800 text-neutral-600 dark:text-neutral-500 hover:border-neutral-600'}`}
            >
              All Content
            </button>
            <button 
              onClick={() => setFilter('case_study')}
              className={`px-4 py-2 text-xs font-bold uppercase tracking-widest border transition-colors ${filter === 'case_study' ? 'border-[#3F618C] text-[#3F618C] bg-[#3F618C]/10' : 'border-neutral-800 text-neutral-600 dark:text-neutral-500 hover:border-neutral-600'}`}
            >
              Case Studies
            </button>
            <button 
              onClick={() => setFilter('blog')}
              className={`px-4 py-2 text-xs font-bold uppercase tracking-widest border transition-colors ${filter === 'blog' ? 'border-[#3F618C] text-[#3F618C] bg-[#3F618C]/10' : 'border-neutral-800 text-neutral-600 dark:text-neutral-500 hover:border-neutral-600'}`}
            >
              Blogs
            </button>
          </div>
          ...
          <div className="relative w-full md:w-72">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-600 dark:text-neutral-500" />
            <input 
              type="text" 
        ...
        <div className="bg-black border border-neutral-800 flex flex-col">
          {isLoading ? (
            ...
            <div className="p-12 text-center text-neutral-600 dark:text-neutral-500 text-sm">
            ...
            filteredItems.map((item, index) => (
              <div 
                key={item.id} 
                className={`flex items-center justify-between p-4 hover:bg-black/5 dark:bg-white/5 transition-colors group cursor-pointer ${index !== filteredItems.length - 1 ? 'border-b border-neutral-800/50' : ''}`}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-neutral-900 dark:text-white font-bold group-hover:text-[#3F618C] transition-colors">{item.title}</h3>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[10px] uppercase tracking-widest text-neutral-600 dark:text-neutral-500">{item.type === 'blog' ? 'Blog' : 'Case Study'}</span>
